...
def receive_messages():
    while True:
        try:
            ...
            save_chat_log(f"Server ({server_username}): {decrypted_message} | Encrypted: {encrypted_message.hex()}")
        except Exception as e:
            messagebox.showerror("Reception Error", f"Error receiving message: {e}")
            break

def save_chat_log(entry):
    with open("client_chat_log.txt", "a") as log_file:
...
ip_entry.insert(0, "127.0.0.1")  # Default localhost IP for testing
...
port_entry.insert(0, "12345")  # Default port for testing
